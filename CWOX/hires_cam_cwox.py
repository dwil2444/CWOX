...
from .base_explainer.attribution.grad_cam import hires_cam


class hires_cam_cwox():
    r"""

    Produces cluster heatmaps with **HiRES-CAM** as base explainer. The back-propagation target is changed to: 
    ...
    def __call__(self, image, targets):
        """Call function for `hires_cam_cwox`."""
        poss=targets
        probs = self.model(image).softmax(-1)
        grad = torch.zeros_like(probs)
        poss_p = probs[0, poss]
        grad[0, poss] = poss_p / poss_p.sum()
        saliency = hires_cam(self.model, image, grad, saliency_layer=self.layer,resize = True)[0,0]
